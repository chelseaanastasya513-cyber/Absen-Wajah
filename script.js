...
async function startCamera() {
  try {
    ...
    console.error(error);
    showNotification('Gagal mengaktifkan kamera. Periksa izin atau perangkat.');
  }
}
...
function capturePhoto() {
  ...
  imageCapture.takePhoto().then(blob => {
    ...
    photoPreviewWrapper.style.display = 'block';
    submitAttendanceButton.disabled = false;
    showNotification('Foto berhasil diambil (cadangan).');
  });
}
...
async function submitAttendance() {
  ...
  if (!name) {
    showNotification('Silakan masukkan nama mahasiswa terlebih dahulu.');
    return;
  }
  ...
  try {
    ...
    submitAttendanceButton.disabled = !capturedImageBase64;
